...
concept domain_class = requires(T a, T b, typename T::value_type val)//NOLINT(readability-identifier-length)
{
  ...
  { a.is_universal() } -> std::same_as<bool>;
  { a.empty() } -> std::same_as<bool>;
  { a.is_singleton() } -> std::same_as<bool>;
  ...
  { min(a) } -> std::same_as<typename T::value_type>;
  { max(a) } -> std::same_as<typename T::value_type>;
...
template<domain_class Domain>
inline std::ostream &operator<<(std::ostream &out, const Domain &domain)
{
