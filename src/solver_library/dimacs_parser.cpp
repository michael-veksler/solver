...
void dimacs_parser::parse_header(const std::function<void(unsigned, unsigned)> &construct_problem)
{
  ...
  if (!line_stream || cmd != "p" || format != "cnf") {
    spdlog::error(
      "{}: Invalid dimacs input format, expecting a line prefix 'p cnf ' but got '{}'", m_current_line_num, *line);
  ...
  if (!line_stream || variables < 0 || clauses < 0) {
    spdlog::error(
      "{}: Invalid dimacs input format, expecting a header 'p cnf <variables: unsigned int> "
